...
if ( ! class_exists( 'NEMOFRAME_Settings' ) ) {

	/**
	 * Handles plugin settings and user profile meta fields
	 */
	class NEMOFRAME_Settings extends NEMOFRAME_Module {
		protected $settings;
...
/*
		public function getReadableProps() {
			return self::$readable_properties;
		}

		public function getWriteableProps() {
			return self::$writeable_properties;
		}
*/
		/*
		 * General methods
		 ...
		 * @param array  $value This will be merged with NEMOFRAME_Settings->settings, so it should mimic the structure of the NEMOFRAME_Settings::$default_settings. It only needs the contain the values that will change, though. See WordPress_Plugin_Skeleton->upgrade() for an example.
		 */
		public function __set( $variable, $value ) {
			// Note: NEMOFRAME_Module::__set() is automatically called before this
			...
			update_option( 'nemoframe_settings', $this->settings );
		}
		...
		protected static function get_settings() {
			$settings = shortcode_atts(
				...
				get_option( 'nemoframe_settings', array() )
			);
		...
		/*
			...
			array_unshift( $links, '<a href="options-general.php?page=' . 'nemoframe_settings">Settings</a>' );
			*/
		...
		public static function markup_settings_page() {
			if ( current_user_can( self::REQUIRED_CAPABILITY ) ) {
				echo self::render_template( 'nemoframe-settings/page-settings.php' );
			} else {
		...
		public function register_settings() {
			...
			add_settings_field(
				'nemoframe_field-example1',
				'Example Field 1',
				array( $this, 'markup_fields' ),
				'nemoframe_settings',
				'nemoframe_section-basic',
				array( 'label_for' => 'nemoframe_field-example1' )
			);
			...
			add_settings_field(
				'nemoframe_field-example2',
				'Example Field 2',
				array( $this, 'markup_fields' ),
				'nemoframe_settings',
				'nemoframe_section-advanced',
				array( 'label_for' => 'nemoframe_field-example2' )
			);
		...
		public static function markup_section_headers( $section ) {
			echo self::render_template( 'nemoframe-settings/page-settings-section-headers.php', array( 'section' => $section ), 'always' );
		}
		...
		public function markup_fields( $field ) {
			switch ( $field['label_for'] ) {
				case 'nemoframe_field-example1':
					// Do any extra processing here
					break;
			}

			echo self::render_template( 'nemoframe-settings/page-settings-fields.php', array( 'settings' => $this->settings, 'field' => $field ), 'always' );
		}
		...
		public function validate_settings( $new_settings ) {
			...
			if ( ! is_string( $new_settings['db-version'] ) ) {
				$new_settings['db-version'] = Nemocube::VERSION;
			}
		...
		public static function add_user_fields( $user ) {
			echo self::render_template( 'nemoframe-settings/user-fields.php', array( 'user' => $user ) );
		}
		...
		public static function save_user_fields( $user_id ) {
			...
			update_user_meta( $user_id, 'nemoframe_user-example-field1', $user_fields[ 'nemoframe_user-example-field1' ] );
			update_user_meta( $user_id, 'nemoframe_user-example-field2', $user_fields[ 'nemoframe_user-example-field2' ] );
		}
		...
		public static function validate_user_fields( $user_id, $user_fields ) {
			if ( $user_fields[ 'nemoframe_user-example-field1' ] == false ) {
				$user_fields[ 'nemoframe_user-example-field1' ] = true;
				add_notice( 'Example Field 1 should be true', 'error' );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				$current_field2 = get_user_meta( $user_id, 'nemoframe_user-example-field2', true );

				if ( $current_field2 != $user_fields[ 'nemoframe_user-example-field2' ] ) {
					$user_fields[ 'nemoframe_user-example-field2' ] = $current_field2;
